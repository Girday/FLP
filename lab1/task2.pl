...
avg_grade_per_subject :-
    ...
    forall(member(Subj, Subjects),
           ( findall(Mark, grade(_, _, Subj, Mark), Marks),
             average(Marks, Avg),
             format('Предмет ~w: средний балл = ~2f~n', [Subj, Avg])
           )).
...
failed_per_group :-
    ...
    forall(member(G, Groups),
           ( findall(S, (grade(G, S, _, 2)), Fails),
             sort(Fails, UniqueFails),
             length(UniqueFails, Count),
             format('Группа ~w: не сдало ~w студентов(-а)~n', [G, Count])
           )).
...
failed_per_subject :-
    setof(Subj, G^S^M^grade(G, S, Subj, M), Subjects),
    forall(member(Subj, Subjects),
           ( findall(S, grade(_, S, Subj, 2), Fails),
             sort(Fails, UniqueFails),
             length(UniqueFails, Count),
             format('Предмет ~w: не сдало ~w студентов(-а)~n', [Subj, Count])
           )).
